...
export const ProductsList = () => {
  const {data: products, isLoading: loadingProduct, error} = useGetProductsQuery();
  const [toggleFeaturedProduct] = useToggleFeaturedProductMutation();
  const [deleteProduct] = useDeleteProductsMutation()
  const [loadingStates, setLoadingStates] = useState<{ [key: string]: boolean }>({}); // Состояние загрузки для каждого продукта
